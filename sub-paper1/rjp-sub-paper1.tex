...
In \texttt{W1} it was shown that through a re-normalization of the four wobbling bands in $^{163}$Lu, its wobbling properties can be successfully described. Indeed, bands $TSD_1$ and $TSD_2$ were considered as ground-state \emph{Signature Partner Pairs}, with the former (latter) being the favored (un-favored) signature partner. The two bands are formed by coupling the $i_{13/2}$ valence proton with even-spin (odd-spin) sequences of the triaxial core. $TSD_3$ was the one-phonon excitation built on top of $TSD_2$.  However, a drawback of that model consists in the requirement of a different quasi-particle (the $h_{9/2}$ proton) to align with the even-even triaxial core for the band $TSD_4$. This results in having two sets of deformation parameters that describe the wobbling properties of the nucleus, namely one set for the bands $TSD_{1,2,3}$, and a second one for $TSD_4$.

By comparison, the new \texttt{W2} formalism will 
...
The energy that characterizes the type of wobbling in a nuclear system is the energy of the first excited band (the one-phonon $n_w=1$ wobbling band) relative to the yrast ground band (the zero-phonon $n_w=0$ wobbling band):
\begin{align}
