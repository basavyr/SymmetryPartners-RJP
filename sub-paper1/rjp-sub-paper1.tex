...
\begin{figure}
    \centering
    \includegraphics[scale=0.45]{figs/wobbling_energy_ThExp.pdf}
...
It is instructive to make a quantitative analysis on other relevant quantities which are of interest for deformed nuclei. The comparison between theory and experiment can be considered as another test on the correctness of the \texttt{W2} formalism. As such, the \emph{rotational/angular frequency} $\hbar\omega$ as a function of total angular momentum $I$ is studied for all four bands. One can consider this quantity as the frequency of the electromagnetic ratiation emitted by a rotating nucleus, in correspondence to classical electrodynamics. Any modifications to the nucleonic motion is directly \emph{embedded} in $\hbar\omega$, since the inertial forces depend on the angular frequency. For rotational bands with $\Delta I=2$, the experimental rotational frequency at given angular momentum $I$ is $\hbar\omega_I=\frac{1}{2}\left(E_I-E_{I-2}\right)$.
