...
\end{minipage}
\caption{Contour plots with the energy function $\mathcal{H}$ for a state in $TSD_1$ (left) and a state from $TSD_2$ (right). Calculations were performed with the numerical parameters obtained from the fitting procedure. The red points represent the minimum points for $\mathcal{H}$, and they represent the regions in space where the nucleus has a stable wobbling character. The blue \emph{islands} represent stable motion.}
    \label{contours-12}
\end{figure}
...
\end{minipage}
\caption{Contour plots with the energy function $\mathcal{H}$ for a state in $TSD_3$ (left) and a state from $TSD_4$ (right). Calculations were performed with the numerical parameters obtained from the fitting procedure. The red points represent the minimum points for $\mathcal{H}$, and they represent the regions in space where the nucleus has a stable wobbling character. The blue \emph{islands} represent stable motion.}
    \label{contours-34}
\end{figure}
